...
# Leer datos 
def leer_datos():
    print("Leyendo coordenadas de las ciudades de la Eurocopa")
    df_estadios = pd.read_excel('Libro1.xlsx', sheet_name='Estadios Eurocopa 2024', decimal=',') 
    print("Leyendo coordenadas de los circuitos de la formula 1") 
    df_formula1 = pd.read_excel('Libro1.xlsx', sheet_name='Circuitos Formula 1 2024', decimal=',')
    print("Leyendo coordenadas de las paradas de metro") 
...
if __name__ == '__main__':
    ...
    ga_tsp = GeneticAlgorithmTSP(generations=500, population_size=400, tournamentSize=10, mutationRate=0.02, fit_selection_rate=0.7)

    mejor_ruta, mejor_coste, total_iteraciones = ga_tsp.find_fittest_path(graph)


   # Mostrar resultados
print("Mejor ruta:", [f"Estadio {i+1}: {df_metro.iloc[i]['Lugar']}" for i in mejor_ruta])
...
m = folium.Map(location=(df_metro.iloc[0]['Latitud'], df_metro.iloc[0]['Longitud']), zoom_start=6)
